...
@Component({
  ...
  templateUrl: './transaction-one.page.html',
  styleUrls: ['./transaction-one.page.scss'],
...
export class TransactionOnePage implements OnInit, OnDestroy {
  ...
  errorLikeTOne: any = '';
  successLikeTOne: any = '';
  public async handleLikeContent(item: JualBeliRuangPensiun): Promise<void> {
    if (this.isLoggedIn) {
      const payload = {
  ...
  loadData(event: any, pagination: any) {
    if (this.listData.length != pagination.totalData) {
      setTimeout(() => {
        ...
        this.fetchJualBeliRuangPensiun(10);
  ...
  public fetchJualBeliRuangPensiun(limit: number = 0): void {
    const response = combineLatest([
  ...
  public async handleOpenJualBeliModal() {
    let dataKategori = {
      idKategori: this.dataParams.idKategori,
    };
